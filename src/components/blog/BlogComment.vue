...
<template>
  <div>
    ...
    <span class="text-silver-600 dark:text-silver-300">
      {{
        comment.createdAt.toLocaleDateString('en-CA', { month: 'long', day: 'numeric', year: 'numeric' })
      }}
    </span>
    <p class="mt-2">{{ comment.body }}</p>
    <div v-if="comment.replies.length" class="border-l-primary-700 ml-2 mt-2 flex flex-col gap-2 border-l py-2 pl-4">
    ...
    <div class="mt-4 flex gap-4">
      <div class="cutout bg-silver-600 dark:bg-silver-300 w-fit p-px">
        <div
          class="hover:bg-silver-500/50 hover:dark:bg-silver-700/75 cutout bg-silver-100 dark:bg-silver-950 flex w-fit cursor-pointer items-center gap-2 px-3 py-2 hover:text-white"
        >
          <svg
            class="size-5"
